...
_"A list of things worth doing"_
...
- **Storage bucket isolation**: local dev uses a separate bucket (`item-images-dev`) from production (`item-images`), both in the same Supabase project, switched via the `SUPABASE_STORAGE_BUCKET` env var (already parameterized, no code involved). Mirrors the existing Postgres dev/prod split (local Docker vs. Supabase), just at the bucket level rather than a fully separate project — decided in Phase 6. **Action needed in Phase 8**: Render's production environment config must set `SUPABASE_STORAGE_BUCKET=item-images`, not the dev value.
